...
use crate::pipeline::visitors::common::{IdentList, ModWithTrailing, NoopSynVisitor};
...
pub enum NetabaseRepositoryAttr {
    name(Ident),
    subscriptions(IdentList),
    storage(Ident),
}

impl syn::parse::Parse for NetabaseRepositoryAttr {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if input.peek(syn::token::Paren) {
            ...
            match s.as_str() {
                ...
                _ => Err(syn::Error::new(ident.span(), format!("Unknown NetabaseRepositoryAttr: {}", s))),
            }
        } else {
            Ok(Self::name(ident))
        }
...
pub struct NetabaseRepositoryAttrList(pub Vec<NetabaseRepositoryAttr>);

impl Parse for NetabaseRepositoryAttrList {
    fn parse(input: ParseStream) -> Result<Self> {
        let punctuated = Punctuated::<NetabaseRepositoryAttr, Token![,]>::parse_terminated(input)?;
        Ok(Self(punctuated.into_iter().collect()))
    }
}

pub struct NetabaseRepositoryVisitor<'ast> {
    pub mod_ident: &'ast Ident,
    pub attrs: Vec<NetabaseRepositoryAttr>,
    pub item: &'ast ItemMod,
    pub trailing: proc_macro2::TokenStream,
}

impl<'ast> FlowVisitor<'ast> for NetabaseRepositoryVisitor<'ast> {
...
impl<'ast> FlowVisitorInput for NetabaseRepositoryVisitor<'ast> {}

impl<'ast> FlowValidate for NetabaseRepositoryVisitor<'ast> {
